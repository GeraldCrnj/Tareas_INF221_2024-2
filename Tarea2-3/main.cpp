...
int DistanciaFuerzaBruta(string S1, string S2){
    ...
    if (S1.length()>1 && S2.length()>1 && S1[0]==S2[1] && S1[1]==S2[0]) {
        int transponer = DistanciaFuerzaBruta(S1.substr(2),S2.substr(2)) + costo_trans(S1[0], S2[0]);
...
int main(){
    ...
    cout << "La distancia mínima de edición es: " << costoMinimo << std::endl;
